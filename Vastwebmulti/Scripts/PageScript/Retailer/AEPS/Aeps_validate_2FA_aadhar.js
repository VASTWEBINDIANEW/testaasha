...
var Ftype = 2;//Finger Type 0 for FMR and 1 for FIR
...
function discoverAvdmFirstNode(PortNo) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            //alert(data);
            //
            //$("#txtdevice_aadharInfo").val(data); AJ
            txtdevice_aadharInfo = data;
...
function discoverAvdm_aadhar() {
    ...
    if (SuccessFlag == 0) {
        $("#deviceerror2_aadhar").text("➀ Connection failed Please try again.");

        $("#Devicerror_aadhar").show();
    }
    // closeNav();
    return res;
}
...
function CaptureAvdm_aadhar() {
    var captureJson = {};
    var strWadh = "";
    var strOtp = "";
    Demo();
    txtWadh = "E0jzJ/P8UopUHAieZn8CKqS4WPMi5ZSYXgfnlfkWjrc=";
    if (txtWadh.trim() != "") {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#cap_aadhar").val(JSON.stringify(captureJson));
            txtPidOptions = XML;
            var $doc;
            if (DeviceKonsi == "Mantra") {
                $doc = $.parseXML(data);
            ...
            try {
                ...
                if ($($doc).find('Resp').attr("errCode") == 0) {
                    ...
                    captureJson.iType = null;//$($doc).find('data');
                    ...
                    try {
                        ...
                        $("#device_srno_aadhar").val($($doc).find('DeviceInfo additional_info Param').attr("value"));
                    } catch (err) { }
                    $('#Devicefind_aadhar').show();
  
                    $('#txtdevice_aadhar').val(DeviceKonsi);
                    $("#cap_aadhar").val(JSON.stringify(captureJson));
                    //  $("#scanimage").hide();
                    // $('#AEPSFOUND').show();
                    $("#Devicerror_aadhar").hide();
                    //  $("#fingImgBlack").hide();
                    //  $("#fingImgGreen").show();
                    //   $("#AEPS").submit();
                } else {
                    $('#Devicefind_aadhar').hide();
                    $("#Devicerror_aadhar").show();
                    $('#deviceerror8_aadhar').text($($doc).find('Resp').attr("errInfo"));
                    $('#deviceerror9_aadhar').text("Please Try again");
                    $('#againscanclick_aadhar').text("scan again");
                }
...
function scanfing_aadhar() {
    ...
    if (RdStatus) {
        ...
        //document.getElementById("loadermoney").style.display = "block";
        CaptureAvdm_aadhar();
        $("#loadaeps1_aadhar").hide();
        //  $('#AEPSFOUND').show();
    }
    else {
        $("#loadaeps1_aadhar").hide();
        $("#Devicerror_aadhar").show();
        $("#deviceerror1_aadhar").text(" Device not found (check the following list)");
        $("#deviceerror3_aadhar").text("➁ Check your device connection");
        $("#deviceerror4_aadhar").text("➂ Check device power supply status");
        $("#deviceerror5_aadhar").text("➃ Check if drivers are installed correctly");
        $("#deviceerror6_aadhar").text("➄ You may not have scanned the finger in scan timing");
        $("#deviceerror7_aadhar").text("➅ There may be a connectivity problem with the bank");
        //alert("Device not found!!");
    }
    //alert("Scan done");
}
function bankChange() {
    $("#bank_aadhar").val($("#iin option:selected").text());
}
