...
function discoverAvdmP() {
    ...
    if (SuccessFlag == 0) {
        $("#deviceerror2DMTAirtel").text("➀ Connection failed Please try again.");

        $("#DevicerrorDMTAirtel").show();
    }
    // closeNav();
    return res;
}
...
function CaptureAvdmP() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#capDMTAirtel").val(JSON.stringify(captureJson));
            txtPidOptions = XML;
            var $doc;
            if (DeviceKonsi == "Mantra") {
                $doc = $.parseXML(data);
            ...
            $("#capxmlPDMTAirtel").val(xmlString);
            ...
            try {
                ...
                if ($($doc).find('Resp').attr("errCode") == 0) {
                    ...
                    try {
                        ...
                        $("#device_srnoDMTAirtel").val($($doc).find('DeviceInfo additional_info Param').attr("value"));
                    } catch (err) { }
                    $('#DevicefindDMTAirtel').show();


                    $('#txtdeviceDMTAirtel').val(DeviceKonsi);
                    $("#capDMTAirtel").val(JSON.stringify(captureJson));
                    //  $("#scanimage").hide();
                    // $('#AEPSFOUND').show();
                    $("#DevicerrorDMTAirtel").hide();
                    //  $("#fingImgBlack").hide();
                    //  $("#fingImgGreen").show();
                    //   $("#AEPS").submit();

                } else {
                    $('#DevicefindDMTAirtel').hide();

                    $("#DevicerrorDMTAirtel").show();
                    $('#deviceerror8DMTAirtel').text($($doc).find('Resp').attr("errInfo"));
                    $('#deviceerror9DMTAirtel').text("Please Try again");
                    $('#againscanclickDMTAirtel').text("scan again");
                }
            } catch (err) {
...
function scanfingP() {
    ...
    if (RdStatus) {
        ...
        $("#loadDMTAirtel1").hide();
        $("#DevicerrorDMTAirtel").show();
        $("#deviceerror1Airtel").text(" Device not found (check the following list)");
        $("#deviceerror3DMTAirtel").text("➁ Check your device connection");
        $("#deviceerror4DMTAirtel").text("➂ Check device power supply status");
        $("#deviceerror5DMTAirtel").text("➃ Check if drivers are installed correctly");
        $("#deviceerror6DMTAirtel").text("➄ You may not have scanned the finger in scan timing");
        $("#deviceerror7DMTAirtel").text("➅ There may be a connectivity problem with the bank");
