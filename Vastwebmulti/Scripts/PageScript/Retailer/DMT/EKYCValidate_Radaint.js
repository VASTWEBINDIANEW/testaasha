...
function discoverAvdmP() {
    ...
    if (SuccessFlag == 0) {
        $("#deviceerror2DMT1").text("➀ Connection failed Please try again.");
...
function CaptureAvdmP() {
    ...
    txtWadh = "TF/lfPuh1n4ZY1xizYpqikIBm+gv65r51MFNek4uwNw=";
    if (txtWadh.trim() != "") {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#capDMT1").val(JSON.stringify(captureJson));
            txtPidOptions = XML;
            var $doc;
            if (DeviceKonsi == "Mantra") {
                $doc = $.parseXML(data);
            ...
            try {
                ...
                if ($($doc).find('Resp').attr("errCode") == 0) {
                    ...
                    try {
                        ...
                        $("#device_srnoDMT1").val($($doc).find('DeviceInfo additional_info Param').attr("value"));
                    } catch (err) { }
                    $('#DevicefindDMT1').show();
                    ...
                    $("#DevicerrorDMT1").show();
                    $('#deviceerror8DMT1').text($($doc).find('Resp').attr("errInfo"));
                    $('#deviceerror9DMT1').text("Please Try again");
                    $('#againscanclickDMT1').text("scan again");
                }
            } catch (err) {
...
function scanfingP() {
    ...
    if (RdStatus) {
        ...
        $("#deviceerror3DMT1").text("➁ Check your device connection");
        $("#deviceerror4DMT1").text("➂ Check device power supply status");
        $("#deviceerror5DMT1").text("➃ Check if drivers are installed correctly");
        $("#deviceerror6DMT1").text("➄ You may not have scanned the finger in scan timing");
        $("#deviceerror7DMT1").text("➅ There may be a connectivity problem with the bank");
